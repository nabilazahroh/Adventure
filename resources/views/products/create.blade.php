...
<div class="max-w-3xl mx-auto">
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">Isi detail produk baru untuk ditambahkan ke stok inventori.</p>
        </div>
        ...
        <form action="{{ route('products.store') }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <select name="unit_type" id="unit_type" required
                        class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 text-sm">
                    <option value="" disabled {{ old('unit_type') ? '' : 'selected' }}>Pilih satuan</option>
                    <option value="ml" {{ old('unit_type') == 'ml' ? 'selected' : '' }}>Mililiter (ml)</option>
                    <option value="cm" {{ old('unit_type') == 'cm' ? 'selected' : '' }}>Centimeter (cm)</option>
                    <option value="meter" {{ old('unit_type') == 'meter' ? 'selected' : '' }}>Meter (m)</option>
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4"
                          class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 text-sm">{{ old('description') }}</textarea>
            </div>

            <div>
                <label for="purchase_price" class="block text-sm font-medium text-gray-700">Harga Beli</label>
                <input type="text" name="purchase_price" id="purchase_price" inputmode="numeric" value="{{ old('purchase_price') }}" required
                       class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 text-sm">
                <p class="mt-1 text-xs text-gray-500">Contoh input: 300000 atau 300.000 (tanpa huruf)</p>
            </div>

            <div>
                <label for="selling_price" class="block text-sm font-medium text-gray-700">Harga Jual</label>
                <input type="text" name="selling_price" id="selling_price" inputmode="numeric" value="{{ old('selling_price') }}" required
                       class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 text-sm">
                <p class="mt-1 text-xs text-gray-500">Contoh input: 300000 atau 300.000</p>
            </div>

            <div>
                <label for="stock" class="block text-sm font-medium text-gray-700">Jumlah Stok Awal</label>
